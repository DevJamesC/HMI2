...
def main():
  ...
  # set the motor variables
    ...
    while True:
        ...
        if(avoidError!=0 and currentLightValue!=maxLightvalue and currentLightValue<15):  #OBJECT AVOIDANCE PID
            i=0
			if(lookingForLocal==true):
				collisionWhileSearch=True #if the bot is turning in a cicle, it will stop if collision procs
            ...
            if(integral<-1000):
                integral=-1000
        elif(currentLightValue>=35): #stopping the bot once it has reached what is most probably the light source
            ...
            if(currentLightValue<maxLightvalue-3 or foundLocal==False or collisionWhileSearch==False):# if true, stop and turn in a circle till it reaches the point where it's facing localmax. Then set maxlight to localmax.
                ...
                if(spincounter>=200 and localMaxLightValue-3==currentLightValue):
					...
					lookingForLocal==False
                    localMaxLightValue=0
			elif(collisionWhileSearch==True):
				collisionSpinCounter=collisionSpinCounter+1
				if(collisionSpinCounter==50):
					collisionWhileSearch=False
					collisionSpinCounter=0
					foundLocal=False
                ...
                p=p/10
				debug_print("this snippit is triggering") #testing to see if the collisionWhileSearch snippit overrides this, or if it will run in conjuction with it.
                mbMove=tp+p
                mcMove=tp-p
